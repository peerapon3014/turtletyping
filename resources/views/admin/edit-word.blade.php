                    ...
                    <div class="card-body mt-4 ">
                        <form action="{{ url('update-word/'.$words->id) }}" method="post">
                            ...
                            <p>&nbsp คำ: <input type="text" name="words" class="from-control" placeholder=""
                                ...
                                ความยาก: <select id="diff" class="form-select form-select-sm" name="diff"
                                            aria-label=".form-select-sm example" >{{$words->difficulties_id}} 
                                            <option value="1" @if($words->difficulties_id==1)selected @endif >ง่าย</option>
                                            <option value="2" @if($words->difficulties_id==2)selected @endif>ปานกลาง</option>
                                            <option value="3" @if($words->difficulties_id==3)selected @endif>ยาก</option>
                                         </select>&nbsp&nbsp&nbsp
                                ภาษา: <select class="form-select form-select-sm" name="language"
                                    aria-label=".form-select-sm example"  >
                                    <!-- <option selected>เลือกภาษา</option> -->
                                    <option value="1" @if($words->languages_id==1)selected @endif>ไทย</option>
                                    <option value="2" @if($words->languages_id==2)selected @endif>อังกฤษ</option>
                                </select>
